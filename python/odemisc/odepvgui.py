...
class Pvgui(QWidget):
    def __init__(self, pvSource, pvDest,lay_lib_name,lay_cell_name,lay_view_name,title_raw="PV", parent=None):
        ...
        self.__inputData = Inputdata(lay_lib_name,lay_cell_name,lay_view_name)
        self.__mainLayout.addWidget(self.__inputData)
        self.__pvCellName = self.__inputData.getLayCellName()
        ...
        print(self.__inputData.getLayCellName(),file=sys.stderr,flush=True)
    ...
    def updateCheckWidget(self):
        for checkType in self.__mainRunWidges:
            ...
            self.__mainRunWidges[checkType].updatePvResultPath(self.__inputData.getLayCellName())
        self.resize(self.size().width(), self.minimumSizeHint().height())

    def getRuleDeckPath(self):
        print(self.__pvCheckTypesWidget[checkType].ruleDeckPath(),file=sys.stderr,flush=True)


class InputdataOne(QWidget):
    global gui
    def __init__(self, sch_or_lay,lib_name,cell_name,view_name,parent=None):
        super(InputdataOne, self).__init__(parent)
        #        self.layLibNameTxt = QLabel("layout library name")
        self.__libName = QLineEdit(lib_name)
        self.__libName.setReadOnly(True)
        self.__cellName = QLineEdit(cell_name)
        self.__cellName.setReadOnly(True)
        self.__cellName.textChanged.connect(self.updateResultDir)
        self.__viewName = QLineEdit(view_name)
        self.__viewName.setReadOnly(True)
        self.__selectLaySch = QPushButton(f"get {sch_or_lay}", self)
        self.__selectLaySch.setMinimumSize(110, 0)

        self.__inputDataLayout = QHBoxLayout()
        self.__inputDataLayout.addWidget(self.__libName)
        self.__inputDataLayout.addWidget(self.__cellName)
        self.__inputDataLayout.addWidget(self.__viewName)
        self.__inputDataLayout.addWidget(self.__selectLaySch)
...
#        print(self.parent.updateCheckWidget(),flush=True)
    ...
    def getLibName(self):
        return (self.__libName.text())
    def getCellName(self):
        return (self.__cellName.text())
    def getViewName(self):
        return (self.__viewName.text())


class Inputdata(QGroupBox):
    def __init__(self,lay_lib_name,lay_cell_name,lay_view_name):
        ...
        self.__layout = InputdataOne("layout",lay_lib_name,lay_cell_name,lay_view_name)
        self.inputDataLayout1.addWidget(self.__layout, 0, Qt.AlignTop)
        self.__schematic = InputdataOne("schematic","tbd","tbd","tbd")
        self.inputDataLayout1.addWidget(self.__schematic, 0, Qt.AlignTop)

        #        print("a"*10)
        #        print(self.__layout.getLayCellName())
        #        print("b"*10)
        self.setLayout(self.inputDataLayout1)

    def getLayLibName(self):
        return (self.__layout.getLibName())
    def getLayCellName(self):
        return (self.__layout.getCellName())
    def getLayViewName(self):
        return (self.__layout.getViewName())

    def getSchLibName(self):
        return (self.__schematic.getLibName())
    def getSchCellName(self):
        return (self.__schematic.getCellName())
    def getSchViewName(self):
        return (self.__schematic.getViewName())
...
class PVcommonOptions(QDialog):
    ...
    def __init__(self, pvSourceSingle, pvCheckType, pvDest, inputData):
        ...
        self.__pvCellName = inputData.getLayCellName()
...
#        self.__rveWidget.text
    ...
    def __updatePvResultPathCombobox(self):
        ...
        pvType = temp[-2]
        pvCell = temp[-1]
        print(rootPath,file=sys.stderr,flush=True)
        ...
        print(resultDirs,file=sys.stderr,flush=True)
        print(self.__result.text,file=sys.stderr,flush=True)
...
#        print(f'load("/home/study/aa")',file=sys.stderr,flush=True)
        ...
        print(f'cell name is : {layCellName}',file=sys.stderr,flush=True)
        layViewName = self.__inputData.getLayViewName()
...
#        self.__accuracyCombo.setFixedWidth(100)
        if os.path.isdir(f"{pvSourceSingle}/latest/accuracy"):
            ...
            print(self.__accuracyFile,file=sys.stderr,flush=True)
